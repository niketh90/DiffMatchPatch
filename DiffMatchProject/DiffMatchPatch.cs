    ...
    public class diff_match_patch
    {
      ...
      */
        ...
        protected Object[] diff_wordsToChars(string text1, string text2)
        {
            ...
            string chars1 = diff_wordsToCharsMunge(text1, wordArray, lineHash);
            string chars2 = diff_wordsToCharsMunge(text2, wordArray, lineHash);
                                            ...
                                            Dictionary<string, int> lineHash)
        {
            ...
            while (wordEnd < text.Length - 1)
            {
                ...
                wordEnd = wordEndSpace;
                if (wordEndSpace > wordEndNewline && wordEndNewline >= 0)
                {
                    wordEnd = wordEndNewline;
                }
                if (wordEnd == -1)
                {
                    wordEnd = text.Length - 1;
                }
                line = text.JavaSubstring(wordStart, wordEnd + 1);
                wordStart = wordEnd + 1;
        ...
        protected void diff_wordsToLines(ICollection<Diff> diffs,
                      List<string> lineArray)
        {
            StringBuilder text;
            foreach (Diff diff in diffs)
